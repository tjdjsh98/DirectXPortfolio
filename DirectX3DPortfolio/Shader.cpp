...
Shader::Shader(wstring file) : _file(L"..\\Shaders\\" + file) , ResourceBase(ResourceType::Shader)
{
...
void Shader::CreateEffect()
{
	...
	for (UINT t = 0; t < _effectDesc.Techniques; t++)
	{
		...
		_techniques[technique.name] = technique;
	}
...
void Shader::Draw(wstring technique, UINT pass, UINT vertexCount, UINT startVertexLocation)
{
	_techniques[technique].passes[pass].Draw(vertexCount, startVertexLocation);
}

void Shader::DrawIndexed(wstring technique, UINT pass, UINT indexCount, UINT startIndexLocation, INT baseVertexLocation)
{
	_techniques[technique].passes[pass].DrawIndexed(indexCount, startIndexLocation, baseVertexLocation);
}

void Shader::DrawInstanced(wstring technique, UINT pass, UINT vertexCountPerInstance, UINT instanceCount, UINT startVertexLocation, UINT startInstanceLocation)
{
...
void Shader::DrawIndexedInstanced(wstring technique, UINT pass, UINT indexCountPerInstance, UINT instanceCount, UINT startIndexLocation, INT baseVertexLocation, UINT startInstanceLocation)
{
...
void Shader::Dispatch(wstring technique, UINT pass, UINT x, UINT y, UINT z)
{
...
void Shader::PushTweenData(const InstancedTweenDesc& desc)
{
	if (_tweenEffectBuffer == nullptr)
	{
		...
		_tweenBuffer->Create();
		_tweenEffectBuffer = GetConstantBuffer("InstancedTweenBuffer");
	}
...
void Shader::PushGlobalData(const GlobalDesc& desc)
{
