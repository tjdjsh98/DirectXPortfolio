...
HRESULT CommonFileDialogApp::CDialogEventHandler_CreateInstance(REFIID riid, void** ppv)
{
    *ppv = NULL;
...
HRESULT CommonFileDialogApp::_WriteDataToFile(HANDLE hFile, PCWSTR pszDataIn)
{
...
HRESULT CommonFileDialogApp::_WritePropertyToCustomFile(PCWSTR pszFileName, PCWSTR pszPropertyName, PCWSTR pszValue)
{
...
// [PROPERTY]foo=bar[ENDPROPERTY]
// [PROPERTY]foo2=bar2[ENDPROPERTY]
HRESULT CommonFileDialogApp::_WriteDataToCustomFile(PCWSTR pszFileName)
{
    HANDLE hFile = CreateFileW(pszFileName,
        GENERIC_READ | GENERIC_WRITE,
        FILE_SHARE_READ,
...
HRESULT CommonFileDialogApp::BasicFileOpen()
{
    // CoCreate the File Open Dialog object.
    IFileDialog* pfd = NULL;
    HRESULT hr = CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pfd));
    UINT index = 0;
    for (auto desc : c_rgSaveTypes)
    {
        pfd->SetFileTypes(index, &desc);
        index++;
    }
    if (SUCCEEDED(hr))
    {
                                    ...
                                    if (SUCCEEDED(hr))
                                    {
                                        ...
                                        if (SUCCEEDED(hr))
                                        {
                                            ...
                                            if (SUCCEEDED(hr))
                                            {
                                                ...
                                                _fileType = static_cast<FileType>(index);
                                                item->GetDisplayName(SIGDN_FILESYSPATH, &_path);
...
HRESULT CommonFileDialogApp::AddItemsToCommonPlaces()
{
    // CoCreate the File Open Dialog object.
    IFileDialog* pfd = NULL;
    HRESULT hr = CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pfd));
...
HRESULT CommonFileDialogApp::AddCustomControls()
{
    // CoCreate the File Open Dialog object.
    IFileDialog* pfd = NULL;
    HRESULT hr = CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pfd));
...
HRESULT CommonFileDialogApp::SetDefaultValuesForProperties()
{
    // CoCreate the File Open Dialog object.
...
HRESULT CommonFileDialogApp::WritePropertiesUsingHandlers()
{
    // CoCreate the File Open Dialog object.
    IFileSaveDialog* pfsd;
    HRESULT hr = CoCreateInstance(CLSID_FileSaveDialog, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pfsd));
...
HRESULT CommonFileDialogApp::WritePropertiesWithoutUsingHandlers()
{
    // CoCreate the File Open Dialog object.
    IFileSaveDialog* pfsd;
    HRESULT hr = CoCreateInstance(CLSID_FileSaveDialog, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pfsd));
...
int CommonFileDialogApp::Update()
{
